...
export const PropertyInspector: React.FC<Props> = ({ selectedNode }) => {
  ...
  const matLabel =
    project?.material ??
    materialOptions().find(m => m.id === project?.materialId)?.label ??
    'Structural Steel';

  return (
    <div className="border-t border-forge-border/40 glass-panel">
      <div className="px-4 py-3 border-b border-forge-border/40 flex items-center gap-2">
        <Box size={14} className="text-forge-blue" />
        <span className="text-[11px] font-semibold tracking-widest text-forge-text-dim">PROPERTIES</span>
        <HelpHint text={tooltips.properties.panel} />
      </div>
      ...
      <div className="p-2 space-y-2 max-h-[220px] overflow-y-auto">
        ...
        <div>
          <button className="flex items-center gap-1 w-full text-left mb-1">
            ...
            <span className="text-[9px] font-semibold tracking-wider text-forge-text-dim">GEOMETRY</span>
          </button>
          <div className="pl-4 space-y-1">
            {[
              ...
              <div key={i} className="flex items-center justify-between py-1">
                <span className="text-[10px] text-forge-text-muted">{prop.label}</span>
                <span className="text-[10px] font-mono text-forge-text">{prop.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <button className="flex items-center gap-1 w-full text-left mb-1">
            ...
            <span className="text-[9px] font-semibold tracking-wider text-forge-text-dim">MATERIAL</span>
          </button>
          <div className="pl-4 space-y-1">
            {[
              { label: 'Name', value: matLabel },
              ...
              <div key={i} className="flex items-center justify-between py-1">
                <span className="text-[10px] text-forge-text-muted">{prop.label}</span>
                <span className="text-[10px] font-mono text-forge-purple">{prop.value}</span>
              </div>
            ))}
          </div>
        </div>
