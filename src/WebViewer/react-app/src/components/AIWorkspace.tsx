...
export const AIWorkspace: React.FC = () => {
  // --- WebSocket connection for live AI events ---
  ...
  useEffect(() => {
    ...
    // Example: emit a test event on connect
    ws.onopen = () => {
      setWsStatus('open');
      ws.send(JSON.stringify({ event_type: 'screen_action', payload: { action: 'workspace_opened', workspace: 'ai-assistant' } }));
    };
    return () => ws.close();
  }, []);
  ...
  return (
    <div className="h-full flex flex-col bg-forge-black overflow-hidden">
      {/* Live AI WebSocket status */}
      <div className="px-2 py-1 text-[10px] font-mono">
        WebSocket: <span className={wsStatus === 'open' ? 'text-forge-green' : wsStatus === 'error' ? 'text-forge-red' : 'text-forge-yellow'}>{wsStatus}</span>
        {wsResponse && <span className="ml-2 text-forge-accent">AI: {wsResponse}</span>}
      </div>
      {/* Header */}
