        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg border border-forge-border bg-forge-panel">
            ...
            <div className="space-y-2">
              {[
                { check: 'Wall Thickness', result: 'PASS', value: '>2mm' },
                { check: 'Undercuts', result: 'PASS', value: 'None' },
                { check: 'Draft Angles', result: 'WARNING', value: '1.5° (min 2°)' },
                { check: 'Tool Access', result: 'PASS', value: 'All surfaces' },
                { check: 'Surface Finish', result: 'PASS', value: 'Ra 0.8μm' },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between">
                  <span className="text-[9px] text-forge-text-dim">{item.check}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[8px] font-mono text-forge-text-muted">{item.value}</span>
                    <span className={`text-[7px] font-mono ${item.result === 'PASS' ? 'text-forge-green' : 'text-forge-yellow'}`}>
                      {item.result}
                    </span>
                  </div>
                </div>
          ...
          <div className="p-4 rounded-lg border border-forge-border bg-forge-panel">
            <div className="flex items-center gap-1.5 mb-3">
              <Shield size={11} className="text-forge-accent" />
              <span className="text-[9px] font-semibold tracking-widest text-forge-text-dim">QUALITY METRICS</span>
            </div>
            <div className="space-y-3">
              {[
                { label: 'Process Capability (Cpk)', value: '1.67', target: '>1.33', ok: true },
                { label: 'First Pass Yield', value: '97.2%', target: '>95%', ok: true },
                { label: 'Defect Rate', value: '0.3%', target: '<1%', ok: true },
                { label: 'GD&T Compliance', value: '98.8%', target: '>98%', ok: true },
              ].map((item, i) => (
                <div key={i}>
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] text-forge-text-dim">{item.label}</span>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[11px] font-bold font-mono text-forge-text">{item.value}</span>
                      <CheckCircle size={9} className="text-forge-green" />
                    </div>
                  </div>
                  <span className="text-[7px] font-mono text-forge-text-muted">Target: {item.target}</span>
                </div>
              ))}
            </div>
          </div>
