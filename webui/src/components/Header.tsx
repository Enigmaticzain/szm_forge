    ...
    <header className="h-12 bg-forge-surface border-b border-forge-border flex items-center px-4 gap-4 relative z-50">
      ...
      <div className="flex items-center gap-1.5 mr-2">
        <button
          ...
          onClick={onUndo}
          disabled={!canUndo}
          className={`px-2.5 py-1 rounded border text-[10px] font-semibold tracking-wider transition-all ${
            canUndo
              ? 'border-forge-border bg-forge-panel text-forge-text hover:text-forge-accent hover:border-forge-accent/40'
              : 'border-forge-border/40 bg-forge-surface text-forge-text-muted cursor-not-allowed'
          }`}
          title="Undo view change"
        >
          ↺ UNDO
        </button>
        <button
