...
export default function ElectricalView() {
  ...
  const { sensors, electricalField } = useSimulationData();
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-56 border-l border-forge-border bg-forge-surface overflow-y-auto p-2 space-y-2">
        ...
        {sensors
          .filter(s => 
            s.id.includes('volt') || s.id.includes('curr') || s.id.includes('pf') || 
            s.id.includes('thd') || s.id.includes('freq') || s.id.includes('active') || 
            s.id.includes('reactive') || s.id.includes('eff') || s.id.includes('temp')
          )
          .map(d => (
            <div key={d.id} className="glass-panel rounded-lg p-2 flex items-center justify-between">
              <div>
                <div className="text-[8px] text-forge-text-muted">{d.name}</div>
                <div className="text-[11px] font-bold text-forge-text font-mono">{d.value.toFixed(2)}{d.unit}</div>
              </div>
              <div className={`w-1.5 h-1.5 rounded-full ${
                d.status === 'normal' ? 'bg-forge-green' : d.status === 'warning' ? 'bg-forge-warning' : 'bg-forge-danger'
              }`} />
            </div>
          ))}
